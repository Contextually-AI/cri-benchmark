...
three required methods — ``ingest``, ``query``, and ``get_all_facts`` —
...
class NoMemoryAdapter:
    ...
    * :meth:`query` — Always returns an empty list regardless of the topic.
    * :meth:`get_all_facts` — Always returns an empty list.
    ...
    def query(self, topic: str) -> list[StoredFact]:
        ...
        ----------
        topic : str
            The topic to query.  Ignored entirely.
