...
* :meth:`retrieve` — embeds a query string and retrieves the top-*k* most
  similar chunks.
* :meth:`get_events` — returns every stored chunk for memory auditing.
...
.. code-block:: python
    ...
    facts = adapter.retrieve("current occupation")
...
class RAGAdapter:
    ...
    >>> adapter.ingest(messages)
    >>> results = adapter.retrieve("dietary preferences")
    >>> len(results) <= 3
    True
    """
    ...
    def retrieve(self, query: str) -> list[StoredFact]:
        """Retrieve stored chunks most similar to the given query.

        The query string is embedded using ChromaDB's default embedding
        ...
        ----------
        query : str
            A natural-language query string describing the information
