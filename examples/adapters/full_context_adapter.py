...
class FullContextAdapter:
    ...
    * :meth:`ingest` — Stores each user message as a ``StoredFact``
      ...
      are skipped.
    * :meth:`retrieve` — Returns **all** stored facts regardless of the
      query string.  No filtering is performed.
    * :meth:`get_events` — Returns all stored facts (identical to
      ``retrieve`` in this adapter).
    ...
    ... ])
    >>> len(adapter.get_events())
    2
    >>> adapter.retrieve("anything")[0].text
    'I live in Berlin'
    """
    ...
    def retrieve(self, query: str) -> list[StoredFact]:
        ...
        ----------
        query : str
            The query string.  Ignored — all facts are returned regardless.
