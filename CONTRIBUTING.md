...
2. **Docstrings on all public symbols.** Use Google-style or NumPy-style docstrings. At minimum, include a one-line summary. For complex functions, document parameters, return values, and notable exceptions.
   ...
   def query(self, topic: str) -> list[StoredFact]:
       """Retrieve stored facts relevant to the given topic.
...
class MyMemoryAdapter:
    ...
    def query(self, topic: str) -> list[StoredFact]:
        """Retrieve facts relevant to a topic string."""
...
| `query(topic)` | During each dimension's scoring | Return *only* facts relevant to the topic |
| `get_all_facts()` | During memory hygiene audits | Return *every* stored fact |
...
- Open a [GitHub Discussion](https://github.com/cri-benchmark/cri/discussions) for general questions
- Open a [GitHub Issue](https://github.com/cri-benchmark/cri/issues) for bugs or feature requests
