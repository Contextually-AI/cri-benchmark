...
class TestNoMemoryAdapter:
    ...
    def test_ingest_is_noop(
        self, no_memory_adapter: NoMemoryAdapter, messages: list[Message]
    ) -> None:
        """Ingest should succeed silently and store nothing."""
        no_memory_adapter.ingest(messages)
        assert no_memory_adapter.get_all_facts() == []
    ...
    def test_query_returns_empty(
        ...
        """Query should always return an empty list."""
        no_memory_adapter.ingest(messages)
        assert no_memory_adapter.query("location") == []
        assert no_memory_adapter.query("occupation") == []
        assert no_memory_adapter.query("") == []

    def test_get_all_facts_returns_empty(self, no_memory_adapter: NoMemoryAdapter) -> None:
        """get_all_facts should always return an empty list."""
        assert no_memory_adapter.get_all_facts() == []

    def test_query_before_ingest(self, no_memory_adapter: NoMemoryAdapter) -> None:
        """Query before any ingestion should return empty list."""
        assert no_memory_adapter.query("anything") == []
...
class TestFullContextAdapter:
    ...
    def test_ingest_stores_user_messages(
        self, full_context_adapter: FullContextAdapter, messages: list[Message]
    ) -> None:
        """Only user messages should be stored as facts."""
        full_context_adapter.ingest(messages)
        facts = full_context_adapter.get_all_facts()
        # 3 user messages out of 5 total
        assert len(facts) == 3

    def test_ingest_ignores_assistant_messages(
        self, full_context_adapter: FullContextAdapter, messages: list[Message]
    ) -> None:
    ...
    def test_query_returns_all_facts(
        self, full_context_adapter: FullContextAdapter, messages: list[Message]
    ) -> None:
        """Query should return ALL facts regardless of topic."""
        full_context_adapter.ingest(messages)
        # Any topic should return all 3 user-message facts
        result_location = full_context_adapter.query("location")
        result_food = full_context_adapter.query("food")
        result_random = full_context_adapter.query("completely unrelated topic")
        assert len(result_location) == 3
        assert len(result_food) == 3
        assert len(result_random) == 3

    def test_query_returns_copies(
        self, full_context_adapter: FullContextAdapter, messages: list[Message]
    ) -> None:
        """Each query call should return a new list (not the internal one)."""
        full_context_adapter.ingest(messages)
        result1 = full_context_adapter.query("topic")
        result2 = full_context_adapter.query("topic")
        # Same content but different list objects
        assert result1 == result2
        assert result1 is not result2

    def test_get_all_facts_returns_copies(
        self, full_context_adapter: FullContextAdapter, messages: list[Message]
    ) -> None:
        """get_all_facts should return a new list each time."""
        full_context_adapter.ingest(messages)
        facts1 = full_context_adapter.get_all_facts()
        facts2 = full_context_adapter.get_all_facts()
    ...
    def test_ingest_empty_list(self, full_context_adapter: FullContextAdapter) -> None:
        ...
        assert full_context_adapter.get_all_facts() == []

    def test_query_before_ingest(self, full_context_adapter: FullContextAdapter) -> None:
        """Query before ingestion should return empty list."""
        assert full_context_adapter.query("anything") == []

    def test_get_all_facts_before_ingest(self, full_context_adapter: FullContextAdapter) -> None:
        """get_all_facts before ingestion should return empty list."""
        assert full_context_adapter.get_all_facts() == []
    ...
    def test_reset_clears_state(
        ...
        full_context_adapter.ingest(messages)
        assert len(full_context_adapter.get_all_facts()) == 3
        full_context_adapter.reset()
        assert full_context_adapter.get_all_facts() == []
        assert full_context_adapter.query("location") == []

    def test_multiple_ingestions_accumulate(
        self, full_context_adapter: FullContextAdapter, messages: list[Message]
    ) -> None:
        ...
        assert len(full_context_adapter.get_all_facts()) == 3
        full_context_adapter.ingest(messages)
        assert len(full_context_adapter.get_all_facts()) == 6

    def test_reset_then_reingest(
        self, full_context_adapter: FullContextAdapter, messages: list[Message]
    ) -> None:
    ...
    def test_repr_shows_fact_count(
        self, full_context_adapter: FullContextAdapter, messages: list[Message]
    ) -> None:
        full_context_adapter.ingest(messages)
        assert "3" in repr(full_context_adapter)

    def test_stored_facts_are_stored_fact_instances(
        self, full_context_adapter: FullContextAdapter, messages: list[Message]
    ) -> None:
        """All returned facts must be StoredFact instances."""
        full_context_adapter.ingest(messages)
        for fact in full_context_adapter.get_all_facts():
            assert isinstance(fact, StoredFact)
        for fact in full_context_adapter.query("test"):
            assert isinstance(fact, StoredFact)
...
class TestBaselineComparison:
    ...
    def test_query_result_types_consistent(self, messages: list[Message]) -> None:
        """Both adapters should return list[StoredFact] from query."""
        no_mem = NoMemoryAdapter()
        full_ctx = FullContextAdapter()
        ...
        no_mem_result = no_mem.query("location")
        full_ctx_result = full_ctx.query("location")
