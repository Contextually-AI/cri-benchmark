...
def _make_adapter(facts: list[str] | None = None) -> MagicMock:
    ...
    adapter.query.return_value = stored
    adapter.get_all_facts.return_value = stored
    return adapter
...
class TestDBUDimension:
    ...
    def test_no_belief_changes(self) -> None:
        ...
        result = asyncio.get_event_loop().run_until_complete(
            DBUDimension().score(adapter, gt, judge)
        )
    ...
    def test_adapter_query_called_with_correct_topics(self) -> None:
        """Adapter.query is called with each belief change's query_topic."""
        changes = [
            BeliefChange(
                fact="f1",
                old_value="a",
        ...
        calls = [c.args[0] for c in adapter.query.call_args_list]
