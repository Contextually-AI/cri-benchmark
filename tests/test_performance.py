...
class BrokenAdapter:
    """An adapter missing the query method — should fail validation."""
...
class TestInstrumentedAdapterProtocol:
    ...
    def test_has_get_all_facts(self) -> None:
        adapter = InstrumentedAdapter(SimpleTestAdapter())
...
class TestInstrumentedAdapterDelegation:
    ...
    def test_ingest_delegates(self) -> None:
        ...
        assert len(inner.get_all_facts()) == 2
    ...
    def test_query_returns_empty_for_unknown(self) -> None:
        ...
        adapter = InstrumentedAdapter(inner)
        adapter.ingest(_make_messages())
        results = adapter.query("quantum physics")
        assert results == []
...
class TestInstrumentedAdapterTiming:
    ...
    def test_query_records_latency(self) -> None:
        adapter = InstrumentedAdapter(SimpleTestAdapter())
        adapter.ingest(_make_messages())
        adapter.query("software")
        assert len(adapter.query_records) == 1
        assert adapter.query_records[0].latency_ms >= 0.0
        assert adapter.query_records[0].topic == "software"

    def test_get_all_facts_records_latency(self) -> None:
        adapter = InstrumentedAdapter(SimpleTestAdapter())
        adapter.ingest(_make_messages())
        adapter.get_all_facts()
        assert len(adapter.get_all_facts_records) == 1
        assert adapter.get_all_facts_records[0].latency_ms >= 0.0
        assert adapter.get_all_facts_records[0].fact_count == 2

    def test_multiple_queries_recorded(self) -> None:
        adapter = InstrumentedAdapter(SimpleTestAdapter())
        adapter.ingest(_make_messages())
        adapter.query("software")
        adapter.query("hiking")
        adapter.query("unknown")
        assert len(adapter.query_records) == 3
    ...
    def test_slow_query_measures_delay(self) -> None:
        adapter = InstrumentedAdapter(SlowTestAdapter(delay_ms=50.0))
        adapter.ingest(_make_messages())
        adapter.query("software")
        assert adapter.query_records[0].latency_ms >= 10.0
...
class TestIntegration:
    ...
    def test_full_pipeline(self) -> None:
        ...
        adapter.query("software engineer")
        adapter.query("hiking")
        adapter.query("unknown topic")
    ...
    def test_instrumented_adapter_does_not_affect_scoring(self) -> None:
        ...
        raw_results = raw.query("software")
        inst_results = instrumented.query("software")
        assert len(raw_results) == len(inst_results)
        assert raw_results[0].text == inst_results[0].text

        # Full fact dump should match
        raw_all = raw.get_all_facts()
        inst_all = instrumented.get_all_facts()
