...
class TestFullContextAdapterIntegration:
    ...
    async def test_full_context_query_returns_all_facts(
        ...
        """FullContextAdapter.query() should return all stored facts."""
        adapter = FullContextAdapter()
        adapter.ingest(sample_messages)

        results = adapter.query("any topic")
        assert len(results) == len(adapter.get_all_facts())
        ...
        for adapter_class, judge_class in [
            ...
            for dim_name in ["PAS", "DBU", "MEI", "TC", "CRQ", "QRP"]:
                ...
                assert 0.0 <= detail.score <= 1.0, (
                    f"{dim_name} score {detail.score} out of range for "
                    f"{adapter_class.__name__}/{judge_class.__name__}"
                )
            assert 0.0 <= result.cri_result.cri <= 1.0
    ...
    async def test_canonical_dataset_dimensions_exercised(
        ...
        for dim_name in ["PAS", "DBU", "MEI", "TC", "CRQ", "QRP"]:
            ...
            assert detail.total_checks > 0, (
                f"Dimension {dim_name} should have checks in canonical dataset"
            )
