...
from cri.judge import DEFAULT_JUDGE_MODEL, BinaryJudge
...
_ADAPTER_ENTRIES: dict[str, tuple[str, str, str, str | None]] = {
    ...
    "full-context": (
        ...
        "FullContextAdapter",
        "Stores every user message; returns all on query. Upper-bound recall baseline.",
        None,
    ),
    "rag": (
    ...
    "upp": (
        ...
        "UPPAdapter",
        "UPP (Universal Personalization Protocol) adapter. "
        "Requires 'pip install cri-benchmark[upp]'.",
        "upp",
    ),
}
...
def resolve_adapter(name_or_path: str) -> type:
    ...
    if name_or_path in _ADAPTER_ENTRIES:
        ...
        raise click.BadParameter(
            f"Adapter '{name_or_path}' is registered but its dependencies are not installed.{hint}"
        )
    ...
    try:
        ...
        raise click.BadParameter(
            f"Cannot resolve adapter '{name_or_path}'. "
            f"Built-in adapters: {available}. "
            f"Or provide a dotted path like 'module:ClassName'. "
            f"Error: {exc}"
        ) from exc


# ---------------------------------------------------------------------------
# Pipeline execution (new API — used by CLI)
# ---------------------------------------------------------------------------
...
async def run_benchmark(
    ...
    Args:
        ...
        judge_model: LLM model for the BinaryJudge.
    ...
    if verbose:
        console.print(
            f"[green]✓[/green] Dataset loaded: "
            f"{len(dataset.messages)} messages, "
            f"{len(dataset.ground_truth.final_profile)} profile dimensions"
        )
    ...
    if verbose:
        console.print(
            ...
            f"Dimensions: {', '.join(config.enabled_dimensions)}"
            + ("  [bold]+SSI[/bold]" if run_scale_test else "")
        )
    ...
    if not isinstance(adapter, MemoryAdapter):
        raise TypeError(
            ...
            f"Required methods: ingest(), query(), get_all_facts()."
        )
    ...
    if run_scale_test:
        ...
        ssi_result = await compute_ssi(
            ...
            ground_truth=dataset.ground_truth,
            judge_factory=lambda: BinaryJudge(model=judge_model, num_runs=judge_runs),
            config=config,
        )
...
@click.option(
    ...
    show_default=True,
    help="LLM model identifier for the binary judge.",
)
@click.option(
    "--judge-runs",
    default=3,
...
@click.option(
    ...
    help=(
        "Scoring profile: 'core' (PAS, DBU, MEI, TC, CRQ, QRP), "
        "'extended' (+ SFC), or 'full' (+ SSI scale test)."
    ),
)
@click.option(
    "--dimensions",
    ...
    type=str,
    help=(
        "Comma-separated list of dimension codes to evaluate "
        "(e.g., 'PAS,DBU,MEI'). Mutually exclusive with --profile."
    ),
)
@click.option(
    "--scale-test",
...
def list_adapters() -> None:
    ...
    console.print()
    console.print(
        "[dim]You can also pass a dotted Python path as --adapter, "
        "e.g., 'mypackage.module:MyAdapterClass'.[/dim]"
    )
    console.print()
...
def list_datasets() -> None:
    ...
    if not datasets:
        ...
        console.print(
            "[dim]Place datasets in datasets/canonical/ or use "
            "--dataset <path> to specify a custom dataset.[/dim]"
        )
        return
