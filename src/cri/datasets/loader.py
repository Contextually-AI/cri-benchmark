...
CANONICAL_DATASETS_DIR = (
    Path(__file__).resolve().parent.parent.parent.parent / "datasets" / "canonical"
)
...
class DatasetInfo(BaseModel):
    ...
    has_ground_truth: bool = Field(
        description="Whether ground_truth.json exists in the dataset directory"
    )
...
def validate_dataset(dataset: ConversationDataset) -> list[str]:
    ...
    if sorted(msg_ids) != expected_ids:
        errors.append(
            f"Message IDs are not sequential 1..{len(dataset.messages)}; "
            f"got {sorted(msg_ids)[:10]}{'...' if len(msg_ids) > 10 else ''}"
        )
    ...
    if dataset.metadata.message_count != len(dataset.messages):
        errors.append(
            f"metadata.message_count ({dataset.metadata.message_count}) does not match "
            f"actual message count ({len(dataset.messages)})"
        )
    ...
    for i, change in enumerate(dataset.ground_truth.changes):
        if change.changed_around_msg not in msg_id_set:
            errors.append(
                f"Belief change [{i}] references invalid message_id "
                f"changed_around_msg={change.changed_around_msg}"
            )
    ...
    for conflict in dataset.ground_truth.conflicts:
        for mid in conflict.introduced_at_messages:
            if mid not in msg_id_set:
                errors.append(
                    f"Conflict '{conflict.conflict_id}' references invalid "
                    f"introduced_at_messages id={mid}"
                )
