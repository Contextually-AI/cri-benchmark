...
class DatasetGenerator:
    ...
    def generate(self, persona: RichPersonaSpec) -> ConversationDataset:
        ...
        logger.info(
            "Dataset for '%s' complete: %d messages, %d profile dims, %d belief changes, %d conflicts",
            persona.name,
            len(messages),
    ...
    def _profile_dim_to_message(
        ...
        value = dim.value
        value_str = (", ".join(value[:-1]) + f" and {value[-1]}" if len(value) > 1 else value[0]) if isinstance(value, list) else str(value)
