...
1. :meth:`ingest` — Feed a chronological list of conversation messages into
   ...
   extract / store any relevant facts.
2. :meth:`query` — Retrieve stored facts that are semantically relevant to a
   given topic string.  This is used by the benchmark's evaluation dimensions
   (PAS, DBU, MEI, TC, CRQ, QRP) to probe the memory system's understanding.
3. :meth:`get_all_facts` — Return *every* stored fact, regardless of query
   relevance.  This powers memory-hygiene auditing — checking for phantom
   facts, duplication, or forgotten updates.
...
class MemoryAdapter(Protocol):
    ...
    * probe its knowledge through targeted queries (:meth:`query`), and
    * audit the full contents of its memory store (:meth:`get_all_facts`).
    ...
    **not** need to inherit from it.  Simply implement the three methods with
    ...
    |                   | extracted facts.                           |
    +-------------------+-------------------------------------------+
    | ``query``         | Retrieve facts relevant to a topic.       |
    +-------------------+-------------------------------------------+
    | ``get_all_facts`` | Dump the entire fact store for auditing.  |
    +-------------------+-------------------------------------------+
    ...
    def ingest(self, messages: list[Message]) -> None:
        ...
        The benchmark runner calls this method once (or in batches) to provide
        the full conversation history that the memory system must process.
        Messages are supplied in **chronological order** — sorted by
        ``message_id`` / ``timestamp`` — and each message includes:
    ...
    def query(self, topic: str) -> list[StoredFact]:
        """Retrieve stored facts that are relevant to the given topic.

        After ingestion, the benchmark runner calls ``query`` with various
        topic strings to evaluate how well the memory system retrieves
        pertinent knowledge.  Topics are aligned with the benchmark's
        ...
        * **QRP** (Query Response Precision) — general retrieval precision
        ...
        topic.  Returning irrelevant facts penalises precision scores;
        missing relevant facts penalises recall.

        Parameters
        ----------
        topic : str
            A natural-language topic string describing what information is
        ...
        list[StoredFact]
            ...
            system considers relevant to *topic*.  May be empty if the system
    ...
    def get_all_facts(self) -> list[StoredFact]:
        ...
        This method provides a **complete dump** of the memory system's
        internal fact store, regardless of any topic or relevance filter.
        The benchmark runner uses it for **memory hygiene auditing**:
