...
class TCDimension(MetricDimension):
    ...
    name: str = "TC"
    description: str = (
        "Measures how well the system handles temporal evolution of "
        "knowledge, distinguishing current from outdated information."
    )
    ...
    async def score(
        ...
        for tf in temporal_facts:
            ...
            # 1. Query the adapter for facts related to this topic
            stored_facts = adapter.query(tf.query_topic)
            ...
            if tf.should_be_current:
                # Fact should be current → YES means it IS present (pass)
                check_passed = result.verdict is Verdict.YES
            else:
                # Fact should NOT be current → YES means system FAILED
                # (still asserts expired fact), NO means system passed
