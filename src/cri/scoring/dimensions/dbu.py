...
class DBUDimension(MetricDimension):
    ...
    name: str = "DBU"
    description: str = (
        "Measures how well the system updates its beliefs when new "
        "information contradicts or supersedes previous knowledge."
    )
    ...
    async def score(
        ...
        for idx, change in enumerate(changes):
            # Query the adapter for facts related to this belief change
            stored_facts = adapter.query(change.query_topic)
            ...
            passed = (
                recency_result.verdict is Verdict.YES and staleness_result.verdict is Verdict.NO
            )
