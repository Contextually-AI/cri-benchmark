...
def tc_temporal_validity_check(
    ...
    if expected_current:
        question = (
            "Is the fact described above present in the stored facts and "
            "treated as currently valid?"
        )
        note = ""
    else:
        question = (
            "Do the stored facts still assert the fact described above as "
            "currently valid, even though it should no longer be current?"
        )
        note = (
            "NOTE: YES means the system FAILED — it treats an expired / "
            "no-longer-current fact as still valid.\n"
        )
