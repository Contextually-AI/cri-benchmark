...
class Message(BaseModel):
    ...
    timestamp: str = Field(
        description="ISO-8601 timestamp string indicating when the message occurred"
    )
...
class ProfileDimension(BaseModel):
    ...
    value: str | list[str] = Field(
        description="Expected value(s) — a single string or list of valid values"
    )
    query_topic: str = Field(
        description="Topic string used when querying the memory system about this dimension"
    )
...
class BeliefChange(BaseModel):
    ...
    changed_around_msg: int = Field(
        description="Approximate message_id around which the change was introduced"
    )
...
class ConflictScenario(BaseModel):
    ...
    conflicting_statements: list[str] = Field(
        description="The contradictory statements introduced in the conversation"
    )
    correct_resolution: str = Field(description="The expected correct resolution of the conflict")
    resolution_type: Literal["recency", "source_authority", "explicit_correction"] = Field(
        description="Strategy that should be used to resolve the conflict"
    )
    introduced_at_messages: list[int] = Field(
        description="Message IDs where the conflicting statements appear"
    )
...
class TemporalFact(BaseModel):
    ...
    should_be_current: bool = Field(
        description="Whether this fact should be treated as currently valid"
    )
...
class QueryRelevancePair(BaseModel):
    ...
    expected_relevant_facts: list[str] = Field(
        description="Facts that should be retrieved for this query"
    )
    expected_irrelevant_facts: list[str] = Field(
        description="Facts that should NOT be retrieved for this query"
    )
...
class ForgettableFact(BaseModel):
    ...
    reason: str = Field(
        description=(
            "Why this fact should be forgotten — e.g. 'ephemeral_state', "
            "'session_context', 'fully_superseded', 'redundant'"
        )
    )
    mentioned_at_message: int = Field(
        description="Message ID where this fact was originally mentioned"
    )
    should_be_absent_after: int = Field(
        description=("Message ID after which this fact should no longer be stored")
    )
...
class GroundTruth(BaseModel):
    ...
    final_profile: dict[str, ProfileDimension] = Field(
        description="Expected final profile keyed by dimension name"
    )
    changes: list[BeliefChange] = Field(
        description="Ordered list of belief changes throughout the conversation"
    )
    noise_examples: list[NoiseExample] = Field(
        description="Examples of noise messages in the dataset"
    )
    signal_examples: list[SignalExample] = Field(
        description="Examples of signal messages in the dataset"
    )
    conflicts: list[ConflictScenario] = Field(
        description="Conflict scenarios embedded in the conversation"
    )
    ...
    query_relevance_pairs: list[QueryRelevancePair] = Field(
        description="Query-relevance pairs for retrieval precision evaluation"
    )
    forgettable_facts: list[ForgettableFact] = Field(
        default_factory=list,
        description=(
            "Facts that should be forgotten/discarded by the end of the "
            "conversation (used by SFC dimension)"
        ),
    )
...
class CRIResult(BaseModel):
    ...
    dimension_weights: dict[str, float] = Field(
        description="Weights used for each dimension in composite calculation"
    )
    details: dict[str, DimensionResult] = Field(
        description="Detailed per-dimension results keyed by dimension name"
    )
...
class PerformanceProfile(BaseModel):
    ...
    memory_growth_curve: list[tuple[int, int]] = Field(
        description="List of (message_count, facts_stored) data points"
    )
...
class ScoringProfile(str, Enum):
    ...
    - **extended** — Core + SFC (7 dimensions).
...
_PROFILE_DIMENSIONS: dict[ScoringProfile, list[str]] = {
    ...
    ScoringProfile.EXTENDED: ["PAS", "DBU", "MEI", "TC", "CRQ", "QRP", "SFC"],
    ScoringProfile.FULL: ["PAS", "DBU", "MEI", "TC", "CRQ", "QRP", "SFC"],
}
